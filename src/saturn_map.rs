...
impl SaturnMapHeader {
    fn new(width: u32, height: u32, tileset_count:u8, tilesets_size: u32, layer_count: u8, layers_size: u32) -> Self {
        SaturnMapHeader {
            magic: 0x894D4150, 
            version: 2, 
            width, 
            ...
            collision_offset: tilesets_size + layers_size
        }
    }
}
...
impl SaturnMap {
    pub fn build(map: Map, words_per_pallate: u8) -> Result<SaturnMap, String> {
        ...
        let tilesets_count = u8::try_from(tilesets.len()).map_err(|e| e.to_string())?;
        let tilesets_size: u32 = tilesets.iter().map(|f| f.tileset_size).sum();

        let layers = SaturnLayer::build(map.layers(), &tilesets)?;
        let layers_count = u8::try_from(layers.len()).map_err(|e| e.to_string())?;
        let layers_size: u32 = layers.iter().map(|f| f.layer_size).sum();

        let collisions = SaturnCollision::build(width, height, map.layers())?;
        
        let mut header = SaturnMapHeader::new(width, height, tilesets_count, tilesets_size, layers_count, layers_size);
